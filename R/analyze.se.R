...
analyze.se <- function(
    ...
    if (!is.null(crispr.altexp)) {
        tmp <- .get_modality(x, crispr.altexp)
        tmp <- .call(
            quickCrisprQc.se, 
        ...
        .assign_modality(x, crispr.altexp, tmp, env=environment())
    }
    ...
    if (!is.null(rna.altexp)) {
        tmp <- .get_modality(x, rna.altexp)
        lib.sizes <- .extract_or_error(colData(tmp), paste0(rna.qc.output.prefix, "sum"))
        tmp <- .call(
    ...
    if (!is.null(crispr.altexp)) {
        tmp <- .get_modality(x, crispr.altexp)
        lib.sizes <- .extract_or_error(colData(tmp), paste0(crispr.qc.output.prefix, "sum"))
        tmp <- .call(
            normalizeRnaCounts.se, # TODO: add a normalizeCrisprCounts.se.
    ...
    if (!is.null(rna.altexp)) {
        tmp <- .get_modality(x, rna.altexp)
        is.hvg <- .extract_or_error(rowData(tmp), paste0(rna.hvg.output.prefix, "hvg"))
        tmp <- .call(
            runPca.se,
    ...
    if (!is.null(chosen.clusters)) {
        if (!is.null(rna.altexp)) {
            markers$rna <- .call(
                scoreMarkers.se,
                list(.get_modality(x, rna.altexp), assay.type=rna.norm.output.name),
        ...
        if (!is.null(adt.altexp)) {
            markers$adt <- .call(
                scoreMarkers.se,
                list(.get_modality(x, adt.altexp), assay.type=adt.norm.output.name),
                list(groups=chosen.clusters, num.threads=num.threads, block=block),
        ...
        if (!is.null(crispr.altexp)) {
            markers$crispr <- .call(
                scoreMarkers.se,
                list(.get_modality(x, crispr.altexp), assay.type=crispr.norm.output.name),
                list(groups=chosen.clusters, num.threads=num.threads, block=block),
...
#' @importFrom SingleCellExperiment altExp altExpNames altExp<-
.delayify_assays <- function(x) {
    for (i in seq_along(assayNames(x))) {
        assay(x, i, withDimnames=FALSE) <- DelayedArray(assay(x, i, withDimnames=FALSE))
    }
    if (is(x, "SingleCellExperiment")) {
        for (i in seq_along(altExpNames(x))) {
            altExp(x, i, withDimnames=FALSE) <- .delayify_assays(altExp(x, i, withDimnames=FALSE))
        }
    }
    x
}

.extract_or_error <- function(df, name) {
    val <- df[[name]]
    if (is.null(val)) {
...
.get_modality <- function(x, altexp) {
    if (is.na(altexp)) {
        x
    } else {
        altExp(x, altexp)
...
#' @importFrom SingleCellExperiment altExp<-
.assign_modality <- function(x, altexp, y, env, name = "x") {
    if (is.na(altexp)) {
        assign(envir=env, x="x", value=y)
    } else {
        altExp(x, altexp) <- y
